...
class WaCommunicate(QtCore.QObject):
...
class WhatsApp:
    def __init__(self, adb_client,variables,communicate):
    ...
    def setStatus_inMainWindow(self,status):
        self.variables.setStatusBar(status)
        self.communicate.setStatusBar.emit()
    ...
    def extract_msgstore(self, dst_path):
        ...
        for spath in storage_paths:
            ...
            self.setStatus_inMainWindow(("[EMULATOR]:Extracting msgstore database from path: %s", db_path))
    ...
    def register_phone(self, msgstore_path, country_code, phone_no):
        ...
        # Step 2: install
        self.setStatus_inMainWindow("[EMULATOR]:Installing WhatsApp...")
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Cleaning WhatsApp...")
        self.adb_client.shell("rm -rf /sdcard/WhatsApp/Databases/*")

        # Step 3b: move msgstore.db to correct location
        self.setStatus_inMainWindow("[EMULATOR]:Moving extracted database into emulator...")
        self.adb_client.push(msgstore_path, "/sdcard/WhatsApp/Databases/msgstore.db.crypt12")

        self.connect_device()

        self.setStatus_inMainWindow("[EMULATOR]:Grant permission to android WRITE/READ_CONTACTS, WRITE/READ_EXTERNAL_STORAGE")
        for permission in [
    ...
    def _do_verify(self, cc, phone):
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Touching and changing country code TextEdit...")
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Touching and changing phone number TextEdit...")
        if not number_view:
            return False
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Touching registration submit button...")
        if not next_view:
            return False
        next_view.touch()
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Touching OK confirmation button...")
        if not confirm_view:
            return False
        confirm_view.touch()
        return True
    ...
    def _verify_by_sms(self, code_callback,variables):
        self.connect_device()
        while True:
            code = ''
            while len(code) != 6:
                code = code_callback(variables)
            if code is not None:
                if self._try_code(code):
                    return True

                self.setStatus_inMainWindow("[EMULATOR]:Verification code NOT valid!")
            else:
                # Attempt to re-send SMS
                self.setStatus_inMainWindow("[EMULATOR]:Attempting to re-send verification code...")

                try:
                    seconds_to_wait = self._get_countdown_sms()

                    # Got countdown seconds, wait and request new code
                    self.setStatus_inMainWindow(("[EMULATOR]:Waiting %d seconds to request new SMS", seconds_to_wait))
    ...
    def _verify_by_call(self, code_callback,variables):
        self.connect_device()
        ...
        while True:
            ...
            if request_call and countdown_view:
                try:
                    ...
                    # Got countdown seconds, wait and request new code
                    self.setStatus_inMainWindow(("[EMULATOR]:Waiting %d seconds to request a call", seconds_to_wait))
                    time.sleep(seconds_to_wait)
                except WaException:
                    pass
            ...
            if code is not None:
                ...
                # Request call again
                self.setStatus_inMainWindow("[EMULATOR]:Attempting to request a new Call...")
                request_call = True

    def _try_code_adb(self, code):
        self.setStatus_inMainWindow(("[EMULATOR]:Set verify code..."))
    ...
    def _try_code(self, code, use_adb=True):
        ...
        if msg_view:
            self.setStatus_inMainWindow(("[EMULATOR]:Dialog message: %s", msg_view.getText()))
            ...
            while True:
                ...
                self.setStatus_inMainWindow("[EMULATOR]:Waiting for input to unblock...")
                time.sleep(10)
    ...
    def _allow_access(self):
        ...
        self.setStatus_inMainWindow('[EMULATOR]:Touching "Continue" on permissions box...')
        continue_view.touch()
        ...
        for i in range(2):
            ...
            self.setStatus_inMainWindow(('[EMULATOR]:Touch "Allow" (step %d)...', i + 1))
            allow_view.touch()
    ...
    def _automate_accept_eula(self):
        ...
        if msg_view.getText().find("ROM") != -1:
            ok_btn = self._wait_views("android:id/button2")

            if not ok_btn:
                return False

            self.setStatus_inMainWindow('[EMULATOR]:Touching "OK" at custom ROM alert...')
            ok_btn.touch()
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Agreeing to EULA...")
        msg_view.touch()
        return True
    ...
    def _wait_views(self, ids, frequency=3, max_tries=15):
        ...
        for attempt in range(max_tries):
            try:  # Update view
                ...
                self.setStatus_inMainWindow(("{} - Exception while trying to dump views: {}".format(attempt, e)))
    ...
    def complete_registration(self, cc, phone):
        ...
        if not gdrive_msg_view and not skip_btn_view:
            self.setStatus_inMainWindow("[EMULATOR]:Expected Google Drive permission dialog, ignoring..")
        else:
            skip_btn_view.touch()
        ...
        allow_view = self._wait_views("com.android.packageinstaller:id/permission_allow_button", frequency=3,max_tries=5)
        if allow_view:
            self.setStatus_inMainWindow('[EMULATOR]:Touch "Allow"...')
            allow_view.touch()
        ...
        self.setStatus_inMainWindow("[EMULATOR]:Restoring messages... (might take a while)")
        ...
        self.setStatus_inMainWindow(("%s", result_msg_view.getText()))
        return True
